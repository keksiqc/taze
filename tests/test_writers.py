...
class TestWritePyprojectUpdates:
    ...
    def test_updates_gte_constraint(self, tmp_path) -> None:
        p = self._write(
            tmp_path,
            """
            [project]
    ...
    def test_handles_single_quotes(self, tmp_path) -> None:
        p = self._write(
            tmp_path,
            "[project]\ndependencies = ['requests>=2.0.0']\n",
        )
    ...
    def test_multiple_deps(self, tmp_path) -> None:
        p = self._write(
            tmp_path,
            """
            [project]
...
class TestWriteRequirementsUpdates:
    ...
    def test_preserves_inline_comment(self, tmp_path) -> None:
    ...
    def test_preserves_other_lines(self, tmp_path) -> None:
        p = self._write(tmp_path, "requests>=2.0.0\nhttpx>=0.20\n")
        dep = self._req_dep("requests>=2.0.0", "requests", "2.0.0", "3.0.0", 1, ">=")
        dep.bump = "major"
        write_requirements_updates(p, [dep])
